...
import scipy.io
import numpy as np
from models.FIN_models import *
import quaternion
from torch.optim.lr_scheduler import StepLR, ConstantLR, ExponentialLR, MultiStepLR


data = scipy.io.loadmat('Fiducial_data.mat')
tensor_data = torch.tensor(data['Rot_PC'])
sampled_tensor = tensor_data   

data = scipy.io.loadmat('Target_data.mat')
...
class get_loss(nn.Module):
    ...
    def quat2euler_zyx(q):
        ...
        min_values = torch.tensor([-14.3309,-14.3309,-14.3309]) 
...
class Net(nn.Module):
    ...
    def forward(self, coarse, batch_db, mask_ex):
        ...
        # init params
        B = xyz_src.size()[0]
        rotation = torch.eye(3).expand(B, -1, -1).cuda() 
        init_quatern = torch.tensor([1, 0, 0, 0]).expand(B, 4)
        init_translation = torch.from_numpy(np.array([[0., 0., 0.]])).expand(B, 3)#.float()
        ...
        min_trans = torch.tensor([-0.0461,-0.0461,-0.0461])  # Male 20240810
        ...
        for i in range(4):
            ...
            #pred_quat, pred_translate = self.regression[i](R_feats, t_feats, iteration) 
            ...
            #xyz_src_iter = quaternion.torch_quat_transform(pose_pred, xyz_src)
...
batch_size=40  
train_dataset = FiducialPointCloudDataset(sampled_tensor, target_db, end_idx=8260)
val_dataset = FiducialPointCloudDataset(sampled_tensor, target_db, start_idx=12600, end_idx=13000)

# Define dataloaders
train_dataloader = DataLoader(train_dataset, batch_size, shuffle=True)
val_dataloader = DataLoader(val_dataset, batch_size, shuffle=False)

#CT_PCA = (CT_PCA.transpose(1,0)).repeat(batch_size,1,1)
...
constant_scheduler = ConstantLR(optimizer, factor=1.0, total_iters=0)
...
# Create a boolean mask to select points at odd multiples of 4
mask = torch.arange(0, 590) % 8 <= 3
mask_ex = ~mask.view(1, -1, 1).expand(batch_size, -1, 3)
...
for epoch in range(epochs):
    ...
    for batch_data, batch_labels, batch_db in train_dataloader:
        ...
        #point_cloud_data = batch_data[:, :1122, :]
        ...
        #gt_translation = batch_labels[:, 5, :3].float()  
        gt_RT = batch_labels[:, 0:4, :].float()
        
        
        translation, transforms, euler, rotations  = model(coarse, batch_db, mask_ex)

        #euler = get_loss.quaternion_to_euler(gt_quat)
        #rotation_mat = get_loss.euler_to_rotation_matrix(gt_euler)
        ...
        for i in range(4):
            ...
            #loss["euler_{}".format(i)] = F.huber_loss(euler[i], gt_euler, delta=0.05) * 1
            #loss["euler_{}".format(i)] = F.l1_loss(euler[i], gt_euler) * 2
    ...
    with torch.no_grad():
        ...
        for batch_data, batch_labels, batch_db in val_dataloader:
            ...
            for i in range(4):
                # reg loss
                #normalized_rotation = get_loss.normalize_rotation_matrices(rotations[i])
                ...
                #loss["translate_{}".format(i)] = F.mse_loss(transforms[i][:, :4, :3], pick[:, :, :3]) * 20
                #loss["translate_{}".format(i)] = F.l1_loss(transforms[i][:, :4, :3], pick[:, :, :3]) * 1
                loss["translate_{}".format(i)] = get_loss.chamfer_distance(transforms[i][:, :4, :3], pick[:, :, :3]) * 1

                #loss["translation_{}".format(i)] = F.mse_loss(translation[i], gt_RT[:,:3,3:].squeeze(-1), reduction='none')   * 100
                loss["translation_{}".format(i)] = F.mse_loss(translation[i] * 1, gt_RT[:,:3,3:].squeeze(-1) * 1)  * 1
                #loss["rotation_{}".format(i)] = F.l1_loss(torch.matmul(rotations[i], gt_RT[:,:3,:3].transpose(2, 1)), identity)    * 1
                #loss["geodesic{}".format(i)] = get_loss.geodesic_loss(rotations[i], gt_RT[:,:3,:3])    * 1

                loss["quat_{}".format(i)] = get_loss.angular_distance_loss(rotations[i], gt_quat) * 1

                #loss["geodesic{}".format(i)] = get_loss.geodesic_loss(rotations[i], gt_RT[:,:3,:3])    * 10
        ...
        #if (val_loss/len(val_dataloader)  >= 12):
    ...
    print(f"Epoch {epoch+1}/{epochs}, Validation Loss: {val_loss/len(val_dataloader):.4f}, Current LR: {current_lr}")
    '''
    '''
    eulers = torch.cat(best_euler, dim=1).cpu().numpy()


    trans = torch.cat(best_translation, dim=1).cpu().numpy()
    
    #eulerss = best_euler.get("gt_euler").data.cpu().numpy()

    #transs = best_translation.get("gt_trans").data.cpu().numpy()
    '''